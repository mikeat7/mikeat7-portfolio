...
export class ExperienceTracker {
  private static readonly STORAGE_KEY = 'train_ai_experiences';

  /**
   * Get all user experiences from localStorage
   */
  static getAllExperiences(): UserExperience[] {
    try {
  ...
  static getExperienceStats() {
    ...
    const recentCount = experiences.filter(e => 
   ...
   */
  static clearAllExperiences(): void {
   ...
   */
  static getRecentFeedback(limit: number = 10): UserExperience[] {
    const experiences = this.getAllExperiences();
    return experiences
...
if (typeof window !== 'undefined') {
  ...
  (window as any).exportTrainingExperiences = () => {
