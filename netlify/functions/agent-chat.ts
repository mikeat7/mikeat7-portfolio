...
function toAnthropicMessages(history: Array<{role:"user"|"assistant", text:string}>, userNow: string, system: string) {
  // Bedrock Anthropic messages start with no explicit 'system' role; we inline system into the first user message.
...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    // Accept BOTH shapes:
    // 1) { text, mode, stakes, cite_policy, omission_scan, reflex_profile }
    // 2) { history:[{role,text}...], text, ... }  (history-aware chat turn)
    const {
      text = "",
    ...
    // Tool traces placeholder to match your UI contract
    return {
      statusCode: 200,
