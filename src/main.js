...
import { validateCodex, buildHandshake, emitTelemetry, } from "@/lib/codex-runtime";
// --- Codex validation + lightweight runtime init ---
const codex = codexJson;
...
window.__clarityArmorCodex = {
    codex_version: codex.codex_version,
    handshake: defaultHandshake,
};
// Log Agent base for awareness (frontend → AWS)
const AGENT_BASE = import.meta.env?.VITE_AGENT_API_BASE;
if (!AGENT_BASE) {
    console.warn("[startup] VITE_AGENT_API_BASE is not set — AWS agent calls will be disabled until you set it (e.g., in .env.local).");
}
else {
...
ReactDOM.createRoot(document.getElementById("root")).render(_jsx(React.StrictMode, { children: _jsx(VXProvider, { children: _jsx(App, {}) }) }));
