...
import { ArrowLeft, Target, CheckCircle, AlertTriangle, Search } from 'lucide-react';


const DetectBullshit: React.FC = () => {
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-lg text-gray-600">
            Learn to identify language designed to impress without regard for truth
          </p>
        </div>
              ...
              {practiceExamples.map((example, index) => (
                <div key={example.id} className="border border-gray-200 rounded-lg p-6">
                  <div className="flex items-start gap-3 mb-4">
                    <Search className="w-5 h-5 text-purple-500 mt-1" />
                  ...
                  <textarea
                    className="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-purple-500 focus:border-purple-500 mb-4"
                    rows={3}
                    placeholder="Identify the bullshit warning signs in this statement..."
                    value={userAnswers[example.id] || ''}
                    onChange={(e) => setUserAnswers(prev => ({ ...prev, [example.id]: e.target.value }))}
                  />
