...
const BullshitPatternViewer: React.FC = () => {
  ...
  const categories = ['all', ...new Set(manipulationPatterns.map((p: any) => p.category))];
  const severities = ['all', 'low', 'medium', 'high', 'critical'];

  const filteredPatterns = manipulationPatterns.filter((pattern: any) => {
    const matchesSearch =
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-4 text-gray-900">Manipulation Pattern Taxonomy</h1>
        ...
        <div className="grid gap-6">
          {filteredPatterns.map((pattern: any) => {
            const indicators: string[] = pattern.semantic_indicators ?? [];
            const clues: string[] = pattern.context_clues ?? [];
            const examples: string[] = pattern.examples ?? [];
            const detectionConfidence: number = Number(pattern.detection_confidence ?? 0);
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h2 className="text-xl font-semibold text-gray-900 mb-2">{pattern.name}</h2>
