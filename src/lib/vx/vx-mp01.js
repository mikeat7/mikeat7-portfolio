...
const detectComprehensiveManipulation = (text) => {
    const frames = [];
    ...
    manipulationPatterns.forEach(pattern => {
        ...
        if (matchedIndicators.length > 0) {
            ...
            const hasContext = (pattern.context_clues || pattern.context_dependencies || []).some(clue => lowered.includes(clue.toLowerCase()));
