...
export function detectSemanticPatterns(text) {
    const frames = [];
    ...
    SEMANTIC_PATTERNS.forEach((pattern) => {
        const hasIndicator = pattern.indicators.some((indicator) => lowered.includes(indicator.toLowerCase()));
        if (hasIndicator) {
...
function getManipulationTactic(patternId) {
    const tactics = {
