...
export function analyzeInquiryContext(text) {
    const lowered = text.toLowerCase();
    return {
        hasMethodologyLanguage: LEGITIMATE_INQUIRY_PATTERNS.methodology.some(pattern => lowered.includes(pattern.toLowerCase())),
        hasIntellectualHumility: LEGITIMATE_INQUIRY_PATTERNS.intellectualHumility.some(pattern => lowered.includes(pattern.toLowerCase())),
        hasProcessOrientation: LEGITIMATE_INQUIRY_PATTERNS.processOriented.some(pattern => lowered.includes(pattern.toLowerCase())),
        hasHiddenAgenda: MANIPULATIVE_INQUIRY_PATTERNS.hiddenAgenda.some(pattern => lowered.includes(pattern.toLowerCase())),
        hasImpliedConspiracy: MANIPULATIVE_INQUIRY_PATTERNS.impliedConspiracy.some(pattern => lowered.includes(pattern.toLowerCase())),
        hasRhetoricalManipulation: MANIPULATIVE_INQUIRY_PATTERNS.rhetoricalManipulation.some(pattern => lowered.includes(pattern.toLowerCase()))
    };
}
export function adjustConfidenceForLegitimateInquiry(baseConfidence, text, detectionType) {
...
export function detectPseudoInquiry(text) {
    const frames = [];
    const context = analyzeInquiryContext(text);
