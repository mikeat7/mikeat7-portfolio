...
export function detectSemanticPatterns(text: string): VXFrame[] {
  ...
  const hasLegitimateHedging =
    /seems|appears|suggests|may|might|could|requires further study|needs investigation/i.test(text);
  ...
  SEMANTIC_PATTERNS.forEach((pattern) => {
    ...
    if (hasIndicator) {
      ...
      const hasContext = pattern.contextClues.some((clue) => lowered.includes(clue.toLowerCase()));
      ...
      // Reduce for methodology language without explicit authority claims
      const hasMethodologyLanguage =
        /could be carried out|should be conducted|requires further study|needs investigation/i.test(
          text
        );
      ...
      // Reduce for reasonable uncertainty when pattern is absolute quantifiers
...
📋 Analysis: Detected manipulation pattern that ${pattern.description.toLowerCase()}. This technique is often used to bypass critical thinking by ${getManipulationTactic(
          pattern.id
        )}.
