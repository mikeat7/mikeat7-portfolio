...
import { buildHandshake, getReflexOrder, shouldTriggerReflex, emitTelemetry, } from '@/lib/codex-runtime';
const DEFAULT_OPTS = {
    mode: '--careful',
    stakes: 'medium',
    useAgent: true,
 ...
 */
const runReflexAnalysis = async (input, opts = {}) => {
    ...
    let localFrames = [];
    try {
        const [confidenceFrames, // VX-CO01
    ...
    for (const f of merged) {
        const { trigger, block } = shouldTriggerReflex(codex, f.reflexId, f.confidence ?? 0, stakes);
        if (block && !blockedBy)
            blockedBy = f.reflexId;
        if (trigger)
            gated.push(f);
    }
    if (blockedBy) {
        gated.unshift({
            ...
            rationale: `Reflex "${blockedBy}" exceeded its block threshold for stakes="${stakes}". ` +
...
function detectReflexClusters(frames, originalInput) {
    const hasLegitSci = /requires further study|needs investigation|warrants research|seems plausible|appears to suggest/i.test(originalInput);
