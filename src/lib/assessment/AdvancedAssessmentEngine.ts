...
export interface AssessmentCriteria {
  ...
  description: string;
  weight: number;
  evaluator: (input: string, context: string) => Promise<number>; // 0-1 score
}
...
class AdvancedAssessmentEngine {
   ...
   */
  async assessNarrativeFraming(userInput: string, scenario: string): Promise<AssessmentResult> {
    const criteria: AssessmentCriteria[] = [
      {
        ...
        weight: 0.3,
        evaluator: async (input: string) => {
          const frameWords = ['frame', 'framing', 'perspective', 'narrative', 'angle', 'emphasis'];
          const frameCount = frameWords.filter(word => 
      ...
      },
      {
        ...
        evaluator: async (input: string) => {
          ...
          const emotionScore = emotionWords.filter(word => 
            input.toLowerCase().includes(word)
          ).length * 0.15;
      ...
      },
      {
        ...
        evaluator: async (input: string) => {
          ...
          const thinkingScore = thinkingWords.filter(word => 
            input.toLowerCase().includes(word)
          ).length * 0.1;
   ...
   */
  async assessEpistemicHumility(userInput: string, scenario: string): Promise<AssessmentResult> {
    const criteria: AssessmentCriteria[] = [
      {
        ...
        evaluator: async (input: string) => {
          ...
          const humilityScore = humilityWords.filter(word => 
            input.toLowerCase().includes(word)
          ).length * 0.1;
          
          // BONUS for phrases that show epistemic humility
          let bonus = 0;
          if (input.toLowerCase().includes('not completely certain')) bonus += 0.3;
          if (input.toLowerCase().includes('could be off')) bonus += 0.2;
          if (input.toLowerCase().includes('roughly') || input.toLowerCase().includes('around')) bonus += 0.2;
          if (input.toLowerCase().includes('i believe') || input.toLowerCase().includes('i recall')) bonus += 0.2;
      ...
      },
      {
        ...
        evaluator: async (input: string) => {
          ...
          const overconfidenceCount = overconfidenceFrames.filter(f => 
            f.reflexId.includes('co01') || f.reflexId.includes('so01')
          ).length;
      ...
      },
      {
        ...
        evaluator: async (input: string) => {
          ...
          const evidenceScore = evidenceWords.filter(word => 
            input.toLowerCase().includes(word)
          ).length * 0.1;
          
          // BONUS for referencing past learning or memory
          let bonus = 0;
          if (input.toLowerCase().includes('recall learning') || input.toLowerCase().includes('remember learning')) bonus += 0.4;
          if (input.toLowerCase().includes('learned') || input.toLowerCase().includes('recall')) bonus += 0.3;
      ...
      },
      {
        ...
        evaluator: async (input: string) => {
          ...
          const perspectiveScore = perspectiveWords.filter(phrase => 
            input.toLowerCase().includes(phrase)
          ).length * 0.15;
          
          // BONUS for acknowledging uncertainty ranges
          let bonus = 0;
          if (input.toLowerCase().includes('two-thirds to three-quarters') || input.toLowerCase().includes('range')) bonus += 0.4;
          if (input.toLowerCase().includes('percentage points') || input.toLowerCase().includes('margin')) bonus += 0.3;
  ...
  private generateNextSteps(overallScore: number, improvements: string[]): string[] {
    const steps: string[] = [];
   ...
   */
  async testAssessmentCalibration(): Promise<void> {
