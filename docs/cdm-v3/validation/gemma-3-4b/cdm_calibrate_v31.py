...
# CDM v3.1 "Gemma Edition" — Elias Rook Small Model Addendum (2026-06-13).
# Round-2 data showed the frontier simultaneity lock never fires on Gemma
# (dH & r coincide only 1.1% of layers; Gini saturated 95%). v3.1 drops Gini,
# adds attention focus ratio (max/mean), and builds the lock on the two
# reliable signals: entropy + output stability.
TH_ENTROPY_DROP = 1.4      # bits, per-layer Delta-H
TH_STABILITY    = 0.70     # output stability (whole-response gate)
TH_FOCUS        = 2.5      # attention focus ratio (max/mean) — replaces Gini
TH_CONVERGENCE  = 0.45     # r(l) <= this (kept, low weight)
LOCK_WINDOW     = 2        # consecutive layers (34-layer stack is short)
LOCK_DENSITY    = 0.20     # >=20% of layers in entropy-collapse
LATE_LAYER_FRAC = 0.59     # layers from here up weighted 1.5x in density
# v3.1 composite weights
W_ENTROPY, W_STABILITY, W_FOCUS, W_CONVERGENCE = 0.40, 0.30, 0.20, 0.10
...
class GlassBox:
    ...
    def analyze(self, prompt: str, response: str) -> dict:
        ...
        # ---- attention Gini (kept for continuity) + focus ratio (v3.1) --
        # Focus ratio = max attention weight / mean — replaces saturated Gini
        # on sliding-window models (Elias v3.1). Per head on the final-token
        # row, averaged over heads.
        ginis, focus = [], []
        for l in range(self.n_layers):
            if l in self._attn:
                ...
                fr_heads = []
                for h in range(self._attn[l].shape[0]):
                    row = self._attn[l][h]
                    m = float(np.mean(row))
                    fr_heads.append(float(np.max(row) / (m + 1e-12)) if m > 0 else 0.0)
                focus.append(float(np.mean(fr_heads)))
            else:
                ginis.append(float("nan"))
                focus.append(float("nan"))
        g0 = next((g for g in ginis if not np.isnan(g)), 0.0)
        gini_delta = [(g - g0) if not np.isnan(g) else float("nan") for g in ginis]

        # ---- output stability (whole-response gate, computed first for lock)
        ...
        # ---- v3.1 lock: per-layer entropy collapse, gated by whole-response
        # stability. Built on the two signals that actually discriminate;
        # Gini/convergence dropped from the lock (round-2 finding).
        stability_ok = stability >= TH_STABILITY
        lock_flags = [
            (delta_h[l] >= TH_ENTROPY_DROP and stability_ok)
            for l in range(self.n_layers)
        ]
        lock_layer = None
        for l in range(self.n_layers - LOCK_WINDOW + 1):
            if all(lock_flags[l : l + LOCK_WINDOW]):
                lock_layer = l
                break
        # late-weighted density (layers from LATE_LAYER_FRAC up count 1.5x)
        late0 = int(self.n_layers * LATE_LAYER_FRAC)
        wsum = sum((1.5 if l >= late0 else 1.0) for l in range(self.n_layers))
        whit = sum((1.5 if l >= late0 else 1.0) for l in range(self.n_layers) if lock_flags[l])
        lock_density = float(whit / wsum)
        is_locked = lock_layer is not None and lock_density >= LOCK_DENSITY

        # ---- composite score (v3.1 weights 40/30/20/10) ----------------
        ...
        mean_focus = float(np.nanmean(focus))
        # focus ratio: sigmoid centered on the TH_FOCUS threshold
        focus_norm = 1.0 / (1.0 + np.exp(-(mean_focus - TH_FOCUS)))
        cdm_score = min(100.0, max(0.0,
            (W_ENTROPY * entropy_norm + W_STABILITY * stability +
             W_FOCUS * focus_norm + W_CONVERGENCE * convergence_norm) * 100))
        ...
        return {
            ...
            "mean_focus_ratio": round(mean_focus, 3),
            "focus_norm": round(focus_norm, 3),
            ...
            "per_layer": {
                ...
                "focus_ratio": [None if np.isnan(f) else round(f, 3) for f in focus],
                "locked": lock_flags,
            },
        }
...
def main():
    ...
    full = {
        "meta": {
            ...
            "thresholds_v31": {
                ...
                "focus_ratio": TH_FOCUS,
                "convergence_ratio": TH_CONVERGENCE,
                "lock_window": LOCK_WINDOW,
                "lock_density": LOCK_DENSITY,
                "weights": [W_ENTROPY, W_STABILITY, W_FOCUS, W_CONVERGENCE],
            },
            "protocol": "Elias Rook CDM v3.1 Gemma Edition (2026-06-13)",
        },
        "responses": responses,
        "results": results,
    }
    ...
    lines = [
        ...
        f"v3.1 (Elias Gemma Edition): dH>={TH_ENTROPY_DROP}, stability>={TH_STABILITY}, "
        f"focus>={TH_FOCUS}, r<={TH_CONVERGENCE}, lock window {LOCK_WINDOW}, "
        f"density>={LOCK_DENSITY}; weights {W_ENTROPY}/{W_STABILITY}/{W_FOCUS}/{W_CONVERGENCE}", "",
    ...
    for tag, m in results.items():
        ...
        lines.append(
            ...
            f"| {m['is_locked']} | {m['lock_layer']} | {m['lock_density']} "
            f"| {m['final_entropy_bits']} | {m['output_stability']} | {m['mean_focus_ratio']} |"
        )
    scores = [m["cdm_score"] for m in results.values()]
    nlock = sum(1 for m in results.values() if m["is_locked"])
    lines += ["", f"Score spread: {min(scores):.1f} – {max(scores):.1f} "
              f"(target per Elias: 30–40 pts). Locks: {nlock}/{len(results)}."]
